...
export const getDomains = () => domains.slice();
...
export async function loadOrganisationDomains() {
  try {
    ...
    const response = await fetch("/v1/integrations/google/domains", {
    ...
    const domainList = data?.data?.domains ?? data?.domains ?? [];
...
export async function ensureDomainsLoaded() {
  if (domainsLoaded) return domains;
...
function findExactDomain(query, domainList) {
...
export async function createDomain(domainName) {
  ...
  if (normalised === "") throw new Error("Domain cannot be empty");
  ...
  const token = await getAccessToken();
  if (!token) throw new Error("Please sign in to create domains");
...
export async function ensureDomainByName(domainName, options = {}) {
  const { allowCreate = true } = options;
  const query = (domainName || "").toString().trim();
  if (!query) return null;

  if (!domainsLoaded) await ensureDomainsLoaded();
...
/**
 * Set up domain autocomplete on an input element.
 * @param {object} options
 */
export function setupDomainSearchInput(options = {}) {
  const {
    input,
    dropdown,
    container,
  ...
  if (!input) return;
  if (input.dataset.domainSearchInitialised === "true") return;
  ...
  if (!dropdown && wrapper) wrapper.appendChild(dropdownEl);
  ...
  const handleSelect = async (domain) => {
    ...
    if (typeof onSelectDomain === "function") await onSelectDomain(domain);
    if (clearOnSelect) {
      input.value = "";
    } else {
  ...
  const handleDropdownKey = async (event) => {
    if (event.key === "ArrowDown") {
      ...
      if (optionItems.length === 0) await renderDropdown(input.value);
      updateFocusedIndex(
        focusedIndex < 0
          ? 0
  ...
  const renderDropdown = async (query) => {
    ...
    const excludedIds =
      typeof getExcludedDomainIds === "function" ? getExcludedDomainIds() : [];
    ...
    if (filtered.length > 0) {
      filtered.forEach((domain) => {
        ...
        optionItems.push({
          el: option,
          onSelect: async () => handleSelect(domain),
        });
        option.addEventListener("focus", () => updateFocusedIndex(optionIndex));
    ...
    if (lowerQuery && resolvedShowCreateOption && resolvedAllowCreate) {
      ...
      optionItems.push({
        el: addOption,
        onSelect: async () => handleCreate(lowerQuery),
      });

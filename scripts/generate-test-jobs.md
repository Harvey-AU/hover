...
`~/.config/hover/auth/session.json` (or `%APPDATA%\Hover\auth\session.json` on
Windows). All load-test scripts will reuse it automatically. Re-run the command
any time you need to refresh the session. Ensure
